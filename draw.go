...
func drawDisp(dc *gg.Context, nick string, now time.Time, items CalItems) {
	...
	dispItems := make([]Event, len(items))
	for i, item := range items {
		dispItems[i].FromT = item.Start.Format("15:04")
		dispItems[i].ToT = item.End.Format("15:04")
